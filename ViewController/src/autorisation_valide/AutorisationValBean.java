                ...
                while (rsi.hasNext()) {
                    ...
                    if(nextRow.getAttribute("case_cocher")!=null){
                        if(Boolean.parseBoolean(nextRow.getAttribute("case_cocher").toString())){
                            ...
                            getEtud.getParamsMap().put("idpers",  nextRow.getAttribute("IdPersonne").toString());
                            getEtud.execute();
                            ...
                            if(row_etud != null){ 
                                ...
                                System.out.println("re " +getDerogationEtud(id_parc_maquette.getInputValue().toString(),id_etud,getAnne_univers()));
                                if(getDerogationEtud(id_parc_maquette.getInputValue().toString(),id_etud,getAnne_univers()) == null){ 
                                    ...
                                    String id_ped = inscriptionValidee(id_parc_maquette.getInputValue().toString(),nextRow.getAttribute("IdPersonne").toString(),id_etud);
                                    if(id_ped != null){ 
                                        OperationBinding op_autorise = getBindings().getOperationBinding("fingAndUpdateAutorise");
                                        op_autorise.getParamsMap().put("id_aut",  Long.parseLong(nextRow.getAttribute("IdAutorise").toString()));
                                ...
                                // validation du Chef pour les primo entrant
                                OperationBinding op_autorise = getBindings().getOperationBinding("fingAndUpdateAutorise");
                                op_autorise.getParamsMap().put("id_aut",  Long.parseLong(nextRow.getAttribute("IdAutorise").toString()));
                                op_autorise.getParamsMap().put("valide",  1);   // validé par le Chef de Scolarité       
                                op_autorise.execute();
