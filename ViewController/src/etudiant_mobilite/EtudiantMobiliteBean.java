...
public class EtudiantMobiliteBean {
    ...
    private String taskFlowId = "/inscription/etudiant_mobilite/dynamic-etud-mob.xml#dynamic-etud-mob";

    public EtudiantMobiliteBean() {
    }
    ...
    public void onChangeNumeroEtud(ClientEvent clientEvent) {
        ...
        sessionScope.put("etudNotNullMobil", 0);
        //tableNotNull
        if(searchString != ""){
            sessionScope.put("etudNotNullMobil", 1);
            try{
                OperationBinding isEtudiant = getBindings().getOperationBinding("isNumEtudiant");
    ...
    public void onChangeNumeroCin(ClientEvent clientEvent) {
        ...
        String searchString=(String)clientEvent.getParameters().get("filterKey");
        System.out.println("searchString searchString "+searchString);
        sessionScope.put("cinNotNullMobil", 0);
        //tableNotNull
        if(searchString != ""){
            sessionScope.put("cinNotNullMobil", 1);
            try{
                //vérifier le numéro d'identification
                OperationBinding isEtudiant = getBindings().getOperationBinding("isNumCin");
    ...
    public void onChangeNumeroTable(ClientEvent clientEvent) {
        ...
        sessionScope.put("tableNotNullMobil", 0);
        sessionScope.put("etudNotNullMobil", 0);
        //tableNotNull
        //num_etud.setValue("");
        if(searchString != ""){
            //num_etud.setValue("");
            sessionScope.put("tableNotNullMobil", 1);
            try{
                OperationBinding is_nouv_bac = getBindings().getOperationBinding("isNumNouvBac");
    ...
    public String onValiderRecherche() {
        ...
        if(rw != null){
            ...
            list_insc_ped.getParamsMap().put("id_pers",  Long.parseLong(rw.getAttribute("IdPersonne").toString()));
            ...
            OperationBinding insc_ped_sem = getBindings().getOperationBinding("getInscPedSem");
            insc_ped_sem.getParamsMap().put("id_pers",  Long.parseLong(rw.getAttribute("IdPersonne").toString()));
            insc_ped_sem.getParamsMap().put("id_sem",  Long.parseLong(getId_sem()));
            insc_ped_sem.getParamsMap().put("id_annee",  Long.parseLong(getAnne_univers()));
            insc_ped_sem.execute();
            
            
            DCIteratorBinding iter_list_insc_ped = (DCIteratorBinding)getBindings().get("InscPedSemEtudMobiliteROIterator");
            ...
            if(row_list_insc_ped != null){
                ...
                sessionScope.put("id_pers_etud_mob",rw.getAttribute("IdPersonne").toString());
                sessionScope.put("insc_ped_sem_etud_mob",row_list_insc_ped.getAttribute("IdInscriptionPedSemestre").toString());
                
                //IdEtudiant
                sessionScope.put("id_etud_mob",row_list_insc_ped.getAttribute("IdEtudiant").toString());
        
                sessionScope.put("TfEtudMobID", "/inscription/etudiant_mobilite/detail-etud-mobilite.xml#detail-etud-mobilite");
            }
            else{
                sessionScope.put("TfEtudMobID", "/inscription/etudiant_mobilite/task-flow-default.xml#task-flow-default");
                
                FacesMessage msg = new FacesMessage(" L'étudiant du prénom & nom : "+rw.getAttribute("Prenoms")+" "+rw.getAttribute("Nom")+" n'est pas inscrit pédagogiquement");
                msg.setSeverity(FacesMessage.SEVERITY_ERROR);
                FacesContext.getCurrentInstance().addMessage(null, msg);
            }
        }
