...
public class DeliberationUeBean {
    ...
    public List<ModeCntrlEc> getModeCntrlEc(Integer fil_ec) {
        ...
        /*lesModCntrl.getParamsMap().put("anne", 2);
        lesModCntrl.getParamsMap().put("sem", 1);
        lesModCntrl.getParamsMap().put("parcours", 2);
        lesModCntrl.getParamsMap().put("sess", 1);*/
    ...
    public HashMap getModeControle(String mode) {
        ...
        OperationBinding lesCtrl = BindingContext.getCurrent()
                                                 .getCurrentBindingsEntry()
                                                 .getOperationBinding("ExecuteWithParamsMod");
        lesCtrl.getParamsMap().put("id_fil_ue_ec", Long.parseLong(mode));
        lesCtrl.execute();

        DCIteratorBinding iter = (DCIteratorBinding) BindingContext.getCurrent()
                                                                   .getCurrentBindingsEntry()
                                                                   .get("ListeModeControlIterator");
        //Create RowSetIterato iterate over viewObject
        RowSetIterator rsi = iter.getViewObject().createRowSetIterator(null);

        while (rsi.hasNext()) {
            Row nextRow = rsi.next();
            list2.put(nextRow.getAttribute("IdModeControleEc").toString(),
    ...
    public HashMap getEc(String idfil) {
        ...
        while (rsi.hasNext()) {
            Row nextRow = rsi.next();
            list2.put(nextRow.getAttribute("IdFiliereUeSemstreEc").toString(),
                      nextRow.getAttribute("Codification").toString());
        }
        rsi.closeRowSetIterator();
        return list2;
    }
    ...
    public String getNote(String num, String mod) {
        OperationBinding lesEtudiantsNotes = BindingContext.getCurrent()
                                                           .getCurrentBindingsEntry()
                                                           .getOperationBinding("ExecuteWithParamsEtudNote");
        AttributeBinding IdFiliereUeSemstre = (AttributeBinding) getBindings().getControlBinding("IdFiliereUeSemstre");
    ...
    public void refresh() {
        ...
        OperationBinding lesEtudiants = BindingContext.getCurrent()
                                                      .getCurrentBindingsEntry()
                                                      .getOperationBinding("ExecuteWithParamsEtudInsc");
  ...
  */
        ...
        if (result == 1) {
            FacesMessage msg =
                new FacesMessage("L'Ue est déja cloturée. Vous pouvez importer le PV de la délibération");
            ...
            if (resultDelib == 1) {
                FacesMessage msg =
                    new FacesMessage("L'Ue est déja délibérée. Vous pouvez effectuer à nouveau la délibération ou cloturer");
                msg.setSeverity(FacesMessage.SEVERITY_WARN);
                FacesContext.getCurrentInstance().addMessage(null, msg);
            } else {
    ...
    public void onDialog(DialogEvent dialogEvent) {
        ...
        if (outcome ==
            Outcome.ok) {
            //commit
            AttributeBinding IdFiliereUeSemstre =
     (AttributeBinding) getBindings().getControlBinding("IdFiliereUeSemstre");
            String action = "O";
    ...
    public String b1_action() {
        ...
        OperationBinding operationBinding = bindings.getOperationBinding("addSelectedTo");
    ...
    public void OnFilUeChanged(ValueChangeEvent valueChangeEvent) {
        // Add event code here...
    }
}
