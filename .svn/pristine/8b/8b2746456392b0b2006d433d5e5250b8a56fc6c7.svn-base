...
public class DecaisserBean {
    ...
    public void onRecherche(ActionEvent actionEvent) {
        ...
        if(row_op_form_hs != null){
            //getLesCompteStrcAnn
            OperationBinding op_les_cmpt = getBindings().getOperationBinding("getLesCompteStrcAnn");
            op_les_cmpt.getParamsMap().put("id_strc",  row_op_form_hs.getAttribute("IdStr"));
            op_les_cmpt.getParamsMap().put("id_annee",  getAnne_univers());
            op_les_cmpt.getParamsMap().put("id_form",  row_op_form_hs.getAttribute("IdFormation"));
            ...
            //la formation n 'est pas payante
        }
    }
    ...
    public void onDialogRechercheCmpte(DialogEvent dialogEvent) {
        // Add event code here...
        DCIteratorBinding iter_op_form_hs = (DCIteratorBinding) BindingContext.getCurrent().getCurrentBindingsEntry().get("lesComptesFormStrcAnROIterator");
        Row row_op_form_hs = iter_op_form_hs.getCurrentRow();
        if(row_op_form_hs != null) {
            setNum_cmpte(row_op_form_hs.getAttribute("NumeroCompte").toString());
            ...
            op_detail_cmpt.getParamsMap().put("id_cmpte", row_op_form_hs.getAttribute("IdCompte"));
    ...
    public void refresh(){
        ...
        if(row_op_form_hs != null){
            //getLesCompteStrcAnn
            OperationBinding op_les_cmpt = getBindings().getOperationBinding("getDetailsOperations");
            op_les_cmpt.getParamsMap().put("id_strc",  row_op_form_hs.getAttribute("IdStr"));
            op_les_cmpt.getParamsMap().put("id_annee",  getAnne_univers());
            op_les_cmpt.getParamsMap().put("id_form",  row_op_form_hs.getAttribute("IdFormation"));
            op_les_cmpt.getParamsMap().put("id_util",  getUtilisateur());
            op_les_cmpt.execute();
            DCIteratorBinding iter_op_det_cmpt = (DCIteratorBinding) BindingContext.getCurrent().getCurrentBindingsEntry().get("DetailOPerationCmptROIterator");
    ...
    public void onInsertBen(ActionEvent actionEvent) {
        ...
        if(row_op_form_hs != null) { 
            ...
            id_strc.setInputValue(row_op_form_hs.getAttribute("IdStructure"));
        ...
        if(row_op_form_hs != null) { 
            ...
            if(sessionScope.get("num_ben_op_var") != null){
                ...
                if(getMontant() != null){
                    ...
                    if(Integer.parseInt( row_op_form_hs.getAttribute("SoldeActuel").toString()) >= Integer.parseInt(getMontant())){
    ...
    public void onDialogConfOper(DialogEvent dialogEvent) {
        ...
        Integer res_op = insertOperationDebit(sessionScope.get("montant_var").toString(), "2", id_mode_paie.getInputValue().toString(), row_op_form_hs.getAttribute("IdCompte").toString(), getNum_chq_vir(), getBanque(),  getObjet(), "0", sessionScope.get("num_op_var").toString(), "0",sessionScope.get("num_ben_op_var").toString(),getDate_op()); 
    ...
    @SuppressWarnings("unchecked")
    public Integer insertOperationDebit(String montant,String type,String mode_paie,String compte,String cheq,String banq,String motif_paie,String debiter,String numero_recu,String valider,String beneficiaire,Date date_op){
        ...
        /*if(Integer.parseInt(mode_paie) == 1){
        ...
        if (!op_commit_op.getErrors().isEmpty()) {
            ...
            if(row_op_form_hs != null){ 
                Integer solde = 0;
                if(row_op_form_hs.getAttribute("SoldeActuel") != null){ 
                    solde = ( Integer.parseInt(row_op_form_hs.getAttribute("SoldeActuel").toString()) - Integer.parseInt(montant));
                }
                OperationBinding getUpdateCmpte = getBindings().getOperationBinding("findAndUpdateCompteEtud");
                getUpdateCmpte.getParamsMap().put("id_solde_compte",  row_op_form_hs.getAttribute("IdSoldeCompte"));  
                ...
                if(row_op_form_hs.getAttribute("IdFormation") != null){
                    //getRechCmpteGlob IdHistoriquesStructure
                    OperationBinding op_oper_cmpt_glob = getBindings().getOperationBinding("getRechCmpteGlob");
                    op_oper_cmpt_glob.getParamsMap().put("id_strct",  row_op_form_hs.getAttribute("IdStructure"));
                    op_oper_cmpt_glob.getParamsMap().put("type_cmpte",  "21");      // id_type_compte : formation globale
                    op_oper_cmpt_glob.getParamsMap().put("id_hs",  row_op_form_hs.getAttribute("IdHistoriquesStructure"));
