...
public class NiveauxBean {
    ...
    private RichPopup popupNivFormExist;
    ...
    public void OnEditNiveauClicked(ActionEvent actionEvent) {
        // Add event code here... UtiModifie
    ...
    public void OnDeleteNiveauAction(DialogEvent dialogEvent) {
        ...
        if (dialogEvent.getOutcome().equals(Outcome.yes)) {
            BindingContainer bindings = getBindings();
            AttributeBinding nvIdBinding = (AttributeBinding) bindings.getControlBinding("IdNiveau");
            Integer nv_id = (Integer.parseInt(nvIdBinding.getInputValue().toString()));
            OperationBinding isNivFormExist = bindings.getOperationBinding("IsNiveauFormNivExist");
            isNivFormExist.getParamsMap().put("nv_id", nv_id);
            isNivFormExist.execute();
            DCIteratorBinding iterNivForm = (DCIteratorBinding) bindings.get("isNiveauFormationNiveauExistIterator");
            Row crntnv = iterNivForm.getCurrentRow();
            if (crntnv == null) {
                OnDeleteClicked();
            }else{
                //popup formation exist
                RichPopup popup = this.getPopupNivFormExist();
                RichPopup.PopupHints hints = new RichPopup.PopupHints();
                //empty hints renders dialog in center of screen
                popup.show(hints);
            }
    ...
    public void onEditNiveauAction(DialogEvent dialogEvent) {
        Outcome outcome = dialogEvent.getOutcome();
        if (outcome == Outcome.ok) {
            //commit
            BindingContainer bindings = getBindings();
    ...
    public void setPopupNivFormExist(RichPopup popupNivFormExist) {
        this.popupNivFormExist = popupNivFormExist;
    }

    public RichPopup getPopupNivFormExist() {
        return popupNivFormExist;
    }
}
