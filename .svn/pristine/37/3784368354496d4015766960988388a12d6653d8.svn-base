...
public class ChangeNationaliteBean {
    ...
    private String taskFlowId = "/inscription/changement_nationalite/dynamic.xml#dynamic";

    public ChangeNationaliteBean() {
    }
    ...
    public void onChangeNumeroEtud(ClientEvent clientEvent) {
        ...
        sessionScope.put("etudNotNullNation", 0);
        //tableNotNull
        if(searchString != ""){
            sessionScope.put("etudNotNullNation", 1);
            try{
                OperationBinding isEtudiant = getBindings().getOperationBinding("isNumEtudiant");
    ...
    public void onChangeNumeroCin(ClientEvent clientEvent) {
        ...
        String searchString=(String)clientEvent.getParameters().get("filterKey");
        System.out.println("searchString searchString "+searchString);
        sessionScope.put("cinNotNullNation", 0);
        //tableNotNull
        if(searchString != ""){
            sessionScope.put("cinNotNullNation", 1);
            try{
                //vérifier le numéro d'identification
                OperationBinding isEtudiant = getBindings().getOperationBinding("isNumCin");
    ...
    public void onChangeNumeroTable(ClientEvent clientEvent) {
        ...
        sessionScope.put("tableNotNullNation", 0);
        sessionScope.put("etudNotNullNation", 0);
        //tableNotNull
        //num_etud.setValue("");
        if(searchString != ""){
            //num_etud.setValue("");
            sessionScope.put("tableNotNullNation", 1);
            try{
                OperationBinding is_nouv_bac = getBindings().getOperationBinding("isNumNouvBac");
    ...
    public String onValiderRecherche() {
        ...
        if(rw != null){
            ...
            if(row_list_insc_ped != null){
                ...
                sessionScope.put("TfChangeNationID", "/inscription/changement_nationalite/detail-change-nationalite.xml#detail-change-nationalite");
            }
            else{
                sessionScope.put("TfChangeNationID", "/inscription/changement_nationalite/task-flow-default.xml#task-flow-default");
